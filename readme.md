...
`styled-jsx` defaults to using a single, global store for styles that are added
while rendering. While this works for simple sites (including statically
...
class MyDocument extends Document {
  static async getInitialProps(ctx) {
    ...
    const initialProps = await Document.getInitialProps({
      ...ctx,
      // Override renderPage as documented here:
      // https://nextjs.org/docs/advanced-features/custom-document#customizing-renderpage
      renderPage: () =>
        ctx.renderPage({
          enhanceApp: App => props => (
            // Wrap the App component using a registry that is unique to the
            // current request
    ...
    return {
      ...
      styles: (
        <>
          {initialProps.styles}
          {flushToReact({ registry: styledJsxRegistry })}
        </>
      )
    }
  }

  // render method omitted for brevity
}
